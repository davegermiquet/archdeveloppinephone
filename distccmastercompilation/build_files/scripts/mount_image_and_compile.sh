...
sudo losetup -P /tmp/archimage-docker-loop0  /tmp/setup/pinephonearch.img
...
sudo mkdir /tmp/modified_image
sudo rsync -avh /tmp/image_attached /tmp/modified_image

sudo cp /usr/bin/qemu-aarch64-static /tmp/modified_image/usr/bin/

sudo mkdir -p /tmp/modified_image/tmp/setup/

sudo cp /tmp/setup/setup_distcc.sh /tmp/modified_image/tmp/setup/scripts/
...
sudo chroot /tmp/modified_image/ -c 'cd /tmp/setup/scripts; \
./setup_distcc.sh; \
